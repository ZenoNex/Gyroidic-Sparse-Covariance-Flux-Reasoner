...
def _test_sdss_direct():
    ...
    assert "rows" in data, "SDSS catalog should contain 'rows' key"
...
def _test_ncbi_direct():
    ...
    assert "sequence" in data, "NCBI sequence should contain 'sequence' key"
    assert "accession" in data, "NCBI sequence should contain 'accession' key"
    assert data["accession"] == "AM743169.1", "Accession mismatch"
    assert len(data["sequence"]) > 0, "Sequence string should not be empty"
