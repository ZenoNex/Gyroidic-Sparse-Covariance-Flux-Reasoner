...
Encodes image patches via gyroid-modulated adaptive pooling, and handles 1D spectral inputs:

1. **Input**: Image tensor `[Batch, Channels, H, W]` or 1D spectral tensors (`[N]`) representing `unified_spectral_signature`, `image_fingerprint`, or `audio_harmonics`.
2. **1D Spectral Preprocessing (BWT Reordering)**: If the input is a 1D tensor, `_prepare_image()` applies a Burrows-Wheeler Transform (BWT) inspired sorting (`torch.sort(image)`) to group identical/similar values. This groups related frequency amplitudes together, maximizing spatial structure prior to the 2D projection.
3. **Landscape Reshaping**: The sorted 1D tensor is padded to a square using fractal (reflective) padding, reshaped into a 2D spectral landscape, and upsampled via bilinear interpolation to match the gyroid's resolution.
4. **Residue projection**: Maps the final representation to the same `[Batch, K, Dim//K]` residue space as text.
