...
$$
\dot{\mathcal{X}} = \Pi_{\mathrm{DP}}\left(\mathrm{ADMM}_{\lambda_j}\left[\mathrm{CRT}_{k}\left(\{ \Pi_{\mathrm{MC}}\left(\nabla f_j(\mathbf{c}_j) \oplus \mathbf{L}\right) \bmod m_k \}_{j}\right)\right]\right)
$$
