...
$$\tilde{\Delta}_{j\ell} = \frac{\Delta_{j\ell}}{(1 + \lambda \cdot \text{PAS}_j) \cdot (1 + 3.0 \cdot \text{Trust}_j)}$$

where $\text{PAS}_j \in [0,1]$ is the per-axis Phase Alignment Score from CALM, $\lambda$ = `pas_anisotropy` (default 2.0), and $\text{Trust}_j \in [0,1]$ is the temporal fidelity score. 
...
|---|---|
| `energy > 0.1  ` and `level < max_depth` | Dive deeper (increase )  finer shell |
| `energy < 0.01  ` and `level > 0` | Surface (decrease )  coarser shell |
...
> [!NOTE]
> **Mischief-Dependent Dynamic Depth**: The shell ceiling is expanded beyond `max_depth` using the `get_mischief_dependent_shell_depth()` protocol, allowing highly entropic states (high mischief) to dive deeper than default restrictions.

### Logic Divergence Note
CAQ computes context-aware per-axis step sizes and applies stochastic rounding to $x$ before handoff. The underlying `MetaPolytopeMatrioshka` re-derives its own independent, uniform `effective_delta` step internally during the containment evaluation loop. This results in a cascading step evaluation rather than a single unified grid.
...
| `pas_anisotropy` | 2.0 | PAS scaling factor  |
...
- [SYSTEM_ARCHITECTURE 9.39.4](SYSTEM_ARCHITECTURE.md)  Matrioshka nested polytopes and quantization formalism
