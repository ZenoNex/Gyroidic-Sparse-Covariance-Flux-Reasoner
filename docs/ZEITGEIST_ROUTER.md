...
**Phase**: 18  
**Status**: [OK] Implemented (2026-02-22)  
**Source**: [`src/core/zeitgeist_router.py`](../src/core/zeitgeist_router.py)  
**References**: [ai project report IIVI](ai%20project%20report_2-2-2026.txt)  [SYSTEM_ARCHITECTURE 9.4](SYSTEM_ARCHITECTURE.md)  [BIOMIMETIC_SYNTHESIS_REPORT 4.4](BIOMIMETIC_SYNTHESIS_REPORT.md)
...
```
  ...
  x_t     facet check: |n, x  c| < ?            
                                                               
                  NO      YES                                  
                        
                   INTERIOR        boundary critical?      
                    unchanged     (stress norm > thr)     
                    -    
                                  NO    YES                    
...
| Mode | Condition | Scalars OK |  updates |
|---|---|---|---|
| `interior` | No facet grazing | [OK] Yes | No |
| `grazing` | Grazing zone hit,  = 0 after round | [ERR] No | No |
| `switching` | Grazing zone hit,   0 | [ERR] No | **Yes** |
...
> **Cold-start note**: Immediately after weight initialization, the gate may produce the same  for different inputs (discrete round() can land at the same integer for nearby sigmoid outputs). Non-commutativity diverges as the gate specializes during training.
...
class ZeitgeistState:
    ...
    level        : int               # Matrioshka shell depth _t
...
- **Poincar Projection**: These IDs act as "landmarks" (Gravity Wells) on the Poincar Disk. As the system navigates, trajectories are pulled toward these historical landmarks, ensuring that the **Navigation** of the manifold is anchored to its **Storage** (Fossilized memory).
...
- [SYSTEM_ARCHITECTURE 9.39.5](SYSTEM_ARCHITECTURE.md)  Matrioshka polytopes and Meta-Polytope dynamics
...
## IVST Side-Chaining and Module Dropout (ADMR Solver Updates)

In Phase 24, the ADMR solver integrates **IVST Side-Chaining** and **Instrument-Modulated Module Dropout** directly into its continuous-time Stochastic Differential Equation updates (`stochastic_differential_step` and `fractional_stochastic_differential_step`).

### Mechanics
1. **Chaotic Envelope Boundary**: The composite threshold $y = \cos(\tau / Z)(\sin(30x) + 1)$ is evaluated using the continuous solver time $\tau$ and the recursive Matrioshka shell scale $Z = 2^{-\text{level}}$ (derived from the active shell level $\ell$).
2. **Birkhoff Mass Conservation**: When a dimension's pressure falls below the threshold $y$, its energy is redistributed to the remaining active dimensions (conserving total state energy/mass) prior to dropout.
3. **Lazarus Rehydration**: Dropped-out dimensions are marked with `NaN` (topological refusal) which triggers the `apply_energy_based_stabilization` protocol. The `NaN` values are replaced with honest jitter to rehydrate the state's variance and break the flatline attractor of `-0.780714`.
