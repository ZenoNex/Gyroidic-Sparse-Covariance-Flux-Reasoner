...
<body>
        ...
        <aside class="sidebar right">
            ...
            <div class="video-panel" id="video-panel">
                ...
                <div id="video-dropzone" style="width: 100%; height: 90px; border: 2px dashed rgba(0, 242, 255, 0.3); border-radius: 12px; display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer; margin-bottom: 0.75rem;">
                    ...
                    <div style="font-size: 0.65rem; color: #666; margin-top: 5px; text-align: center; letter-spacing: 0.5px;">DRAG BINARY VIDEO<br><span style="color:#444;">mp4 &bull; avi</span></div>
                    <input type="file" id="video-file-input" style="display:none;" accept="video/mp4,video/avi,.mp4,.avi">
                </div>
    ...
    <script>
        ...
        async function sendMessage(text) {
            ...
            try {
                const payload = {
                    ...
                    commutativity: _resolveCommutativity(),
                };
        ...
        //
        // The old 137-dim histogram format is gone; the backend fingerprint_proj
            ...
            reader.onload = async (e) => {
                ...
                img.onload = () => {
                    ...
                    appendMessage('system',
                        `DYAD INGESTED: ${totalCoeffs}-COEFF CHEBYSHEV FINGERPRINT ` +
                        `(K=${fingerprint.chebyshev_degree}, ` +
                        `${fingerprint.px_width}×${fingerprint.px_height}px).`);
                };
        ...
        function fingerprintToVector(fp) {
            // For the ribbon display: flatten L+Cr+Cb then pad/truncate to 137 bins
            const flat = [...fp.L, ...fp.Cr, ...fp.Cb];
            while (flat.length < 137) flat.push(0);
        ...
        async function processVideoFile(file) {
            const validTypes = ['video/mp4', 'video/avi', 'video/x-matroska', ''];
            const ext = file.name.toLowerCase().slice(file.name.lastIndexOf('.'));
            if (!['.mp4', '.avi', '.mkv'].includes(ext)) {
                appendMessage('system', `VIDEO DYAD REJECTED: Unsupported format.`);
                return;
            }
        ...
        async function commitVideoDyad() {
            ...
            const payload = {
                text: description || `INGEST_VIDEO_DYAD: [CRYPTOGRAPHIC_PARSE]`,
                video_dyad_b64: b64,
                commutativity: commutativity
            };
