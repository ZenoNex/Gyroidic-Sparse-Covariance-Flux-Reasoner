    ...
    <style>
        ...
        * {
        ...
        body.rupture-mode {
            ...
            background-size: 100% 100%, 4px 4px; /* Tiny grid scanlines */
        ...
        .chain-visualizer:empty::after {
            content: "CHAIN EMPTY — DRAG RESIDUES HERE";
            position: absolute;
            top: 50%;
            left: 50%;
        ...
        .panel-e-title { color: var(--terminal-magenta); }
        .panel-f-title { color: #8a2be2; }
    </style>
</head>
...
<body>
    ...
    <header>
        ...
        <!-- Master Commutativity Selector ─────────────────────────────────────
             Controls the global A→B ordering for ALL dyad types simultaneously.
             Per-panel selectors remain for within-panel ordering only.
             Sends 'commutativity' field with every /interact payload. -->
        <div style="display:flex;align-items:center;gap:0.75rem;padding:0.35rem 1.25rem;background:rgba(0,0,0,0.4);border-top:1px solid var(--glass-border);font-family:var(--font-mono);font-size:0.65rem;letter-spacing:1px;">
            <span style="color:#555;">DYAD ORDER:</span>
            <select id="commute-master" style="background:#0a0a0a;border:1px solid #333;color:var(--terminal-blue);font-family:var(--font-mono);font-size:0.65rem;padding:2px 6px;border-radius:3px;outline:none;cursor:pointer;">
            ...
            <span style="color:#555;font-size:0.58rem;">Governs all dyad channels simultaneously</span>
        </div>
    </header>
    ...
    <main>
        ...
        <aside class="sidebar right">
            ...
            <div class="video-panel" id="video-panel">
                ...
                <!-- Drop zone -->
                <div id="video-dropzone" style="width: 100%; height: 90px; border: 2px dashed rgba(0, 242, 255, 0.3); border-radius: 12px; display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer; margin-bottom: 0.75rem;">
                    <div style="font-size: 1.4rem; line-height: 1; color: var(--terminal-blue);">&#128249;</div>
                    <div style="font-size: 0.65rem; color: #666; margin-top: 5px; text-align: center; letter-spacing: 0.5px;">DRAG BINARY MEDIA<br><span style="color:#444;">mp4 &bull; avi &bull; gif &bull; webm</span></div>
                    <input type="file" id="video-file-input" style="display:none;" accept="video/mp4,video/avi,video/webm,image/gif,.mp4,.avi,.webm,.gif">
                </div>
                ...
                <button id="commit-video" style="width: 100%; background: rgba(0, 242, 255, 0.08); border: 1px solid var(--terminal-blue); color: var(--terminal-blue); padding: 6px; border-radius: 4px; font-size: 0.7rem; cursor: pointer; font-weight: 700;" disabled>COMMIT VIDEO DYAD</button>
            ...
            <div class="chain-panel" id="panel-e">
                ...
                <button id="commit-e" style="width: 100%; background: rgba(255, 0, 242, 0.08); border: 1px solid var(--terminal-magenta); color: var(--terminal-magenta); padding: 6px; border-radius: 4px; font-size: 0.7rem; cursor: pointer; font-weight: 700;">COMMIT BI-MODAL BRAID</button>
            ...
            <div class="chain-panel" id="panel-f">
                ...
                <div style="display:flex; gap:0.5rem; margin-bottom:0.75rem;">
                    <button id="clear-chain-f" style="flex:1; background:rgba(255,255,255,0.05); border:1px solid #444; color:#888; padding:4px; border-radius:4px; font-size:0.6rem; cursor:pointer;">CLEAR CHAIN</button>
                </div>

                <button id="commit-f" style="width: 100%; background: rgba(138, 43, 226, 0.08); border: 1px solid #8a2be2; color: #8a2be2; padding: 6px; border-radius: 4px; font-size: 0.7rem; cursor: pointer; font-weight: 700;">EXECUTE MANIFOLD BRAID</button>
    ...
    <script>
        ...
        function handleResponse(data) {
            ...
            if (data.error) {
                ...
                typewriterMessage('system', data.response, curvature);
            } else {
            ...
            // decoration.
            if (data.visualization_b64) {
                const rs = data.retrieval_state || 'FRACTURE';
                ...
                vizImg.src = `data:image/png;base64,${data.visualization_b64}`;
        ...
        function updateMetricsFromDiagnostics(data) {
            ...
            if (data.resonance_score !== undefined) document.getElementById('resonance-score').innerText = data.resonance_score.toFixed(4);
        ...
        async function processImage(file) {
            ...
            reader.onload = async (e) => {
                ...
                img.onload = () => {
                    ...
                    const totalCoeffs = fingerprint.L.length + fingerprint.Cr.length + fingerprint.Cb.length;
                    appendMessage('system',
                        `⬡ DYAD ARMED — ${totalCoeffs}-coeff Chebyshev fingerprint computed locally ` +
                        `(K=${fingerprint.chebyshev_degree}, ${fingerprint.px_width}\u00d7${fingerprint.px_height}px).\n` +
                        `\u25b6 PROMPT: Type a description and press Enter to INGEST.\n` +
                        `\u25b6 ADVANCED: Type 'ASSOCIATE: source <-> target' to link a new dyad manually.`);
                    
                    // Visual signal: arm the input box
        ...
        async function processVideoFile(file) {
            const validTypes = ['video/mp4', 'video/avi', 'video/x-matroska', 'video/webm', 'image/gif', ''];
            ...
            const reader = new FileReader();
            reader.onload = (e) => {
                const b64 = e.target.result; // "data:video/mp4;base64,..."
                state.active_video_dyad = b64;
        ...
        async function commitVideoDyad() {
            ...
            const payload = {
                text: `INGEST_VIDEO_DYAD: [CRYPTOGRAPHIC_PARSE]${description ? ' | ' + description : ''}`,
                video_dyad_b64: b64,
                commutativity: commutativity
            };
