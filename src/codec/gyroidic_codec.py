...
class TextResidueProjector(nn.Module):
    ...
    def forward(self, text: str) -> torch.Tensor:
        ...
        for k in range(self.K):
            ...
            quantized_flat, _ = self.caq(flat_matrix.unsqueeze(0))
...
class GyroidImageProjector(nn.Module):
    ...
    def forward(self, image: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        for k in range(self.K):
            ...
            # Apply CAQ
            quantized_flat, _ = self.caq(projected.unsqueeze(0))
            matrix = quantized_flat.view(self.n, self.n)
...
class NonAbelianCombiner:
    ...
    def combine(
        ...
        """
        Non-abelian combination: Standard Group Multiplication in GL(n).
        """
